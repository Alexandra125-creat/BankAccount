# Bank System — консольное приложение на C++

## О проекте
Консольное приложение для управления банковскими счетами.

## Команда и роли
- **Архитектор (Team Lead)**: Краснова Александра — создание структуры проекта, CMakeLists.txt, организация работы команды, управление репозиторием
- **Разработчик 1 (Developer 1)**: Бричков Захар — реализация класса `BankAccount`
- **Разработчик 2 (Developer 2)**: Бредихин Александр — реализация класса `Bank`
- **Тестировщик (QA Engineer)**: Бричков Захар, Бредихин Александр — написание `main.cpp`, интеграционное тестирование, проверка сценариев
- **Технический писатель (Tech Writer)**: Краснова Александра — документация, README, фиксация диалогов с нейросетью
### Предварительные требования
- WSL2 с дистрибутивом Ubuntu
- Visual Studio Code с расширениями: C/C++, CMake Tools, Remote - WSL
- Компилятор g++ (версия 9 или выше), CMake, Git (установлены в WSL)
## Структура проекта

    BankSystem/
        .gitignore
        CMakeLists.txt
        README.md
        include/
            BankAccount.h
            Bank.h
        src/
            BankAccount.cpp
            Bank.cpp
        main.cpp
## Класс №1 - BankAccount
### Методы
- BankAccount(accNumber, owner, balance)	-Конструктор с параметрами
- deposit(double amount)	- Пополнение счета (возвращает true при успехе)
- withdraw(double amount)	- Снятие средств с проверкой остатка
- getBalance()	- Получение текущего баланса
- getAccountNumber()	- Получение номера счета
- getOwnerName()	- Получение имени владельца
- printStatement()	- Вывод выписки по счету
- printBalance()` — вывод информации о счете (номер, владелец, баланс)
- *Валидация: При пополнении и снятии проверяется, что сумма положительная. При снятии также проверяется достаточность средств.

## Класс №2 - Bank
*   **Управление памятью:** Деструктор освобождает память всех созданных счетов.
*   **Начальный баланс:** При создании счета баланс устанавливается в 0.
*   **Формат номера счета:** Автоматически генерируется в формате `ACC` + 4-значный номер (начиная с 1000).
### Методы
- createAccount(owner)	- Создание нового счета (автоматическая генерация номера)
- findAccount(accNumber)	- Поиск счета по номеру (возвращает указатель или nullptr)
- getTotalBankBalance()	- Суммарный баланс всех счетов
- listAllAccounts()	-Вывод списка всех счетов
- getAccountCount()	- Количество открытых счетов

## Пример работы прораммы
```
========================================
      БАНКОВСКАЯ СИСТЕМА v1.0
========================================

📌 Создание счетов:
Создан счет: ACC1000 для Иван Петров
Создан счет: ACC1001 для Мария Смирнова
Создан счет: ACC1002 для Алексей Иванов

💳 Пополнение счета ACC1000 на 5000 руб.
Номер счета: ACC1000
Имя держателя счета: Иван Петров
Баланс: 5000
💸 Снятие 2000 руб. со счета ACC1001
❌ Недостаточно средств
Номер счета: ACC1001
Имя держателя счета: Мария Смирнова
Баланс: 0

📋 Все счета в банке:
Номер счета: ACC1000
Имя держателя счета: Иван Петров
Баланс: 5000
Номер счета: ACC1001
Имя держателя счета: Мария Смирнова
Баланс: 0
Номер счета: ACC1002
Имя держателя счета: Алексей Иванов
Баланс: 0

💰 Общий баланс банка: 5000 руб.
========================================
      ТЕСТИРОВАНИЕ ЗАВЕРШЕНО
========================================
```

## Проблемы и решения
(диалоги с нейросетью)

### Проблема 1: Генерация уникальных номеров счетов
**Запрос**: "Как лучше генерировать уникальные номера банковских счетов в C++ для консольного приложения?"

**Ответ**: Рекомендовано использовать счетчик в классе Bank, который увеличивается при создании каждого нового счета. Номер формируется как префикс "ACC" + число. Это гарантирует уникальность в пределах одного банка.

### Проблема 2: Обработка ошибок при снятии средств
**Запрос**: "Что должен возвращать метод withdraw при недостатке средств?"

**Ответ**: Метод должен возвращать boolean (true при успехе, false при неудаче) и НЕ изменять баланс, если операция невозможна. Это позволяет вызывающему коду корректно реагировать на ситуацию.
## Инструкция по сборке
```bash
mkdir build && cd build
cmake ..
make
./BankSystem
